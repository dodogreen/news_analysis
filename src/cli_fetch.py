"""CLI: fetch and filter news articles, save to JSON.
...
def _serialize(article: FilteredArticle) -> dict:
    return {
        "title": article.title,
        "link": article.link,
...
def main() -> None:
    ...
    # Serialize to JSON
    data = [_serialize(a) for a in filtered]
    with open(args.output, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    logger.info("Saved %d articles to %s", len(data), args.output)
    print(f"Fetched {len(data)} articles → {args.output}")
